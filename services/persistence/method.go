...
func (r *rpcxServer) SaveMsgToDb(ctx context.Context) saveMsgToDbFn {
	...
	return func(ctx context.Context, args SaveMsgToDbArgs, reply *SaveMsgToDbReply) error {
		result, err := rethinkdb.Table(config.RethinkMsgDb).
			...
			RunWrite(rdb)
		if err != nil {
			r.GetLogger().Error("SaveMsgToDb rethinkdb.Insert Err", fmt.Sprintf("result:%+v arg:%+v err:%v", result, args, err))
		}
		return err
	}
}
...
func (r *rpcxServer) GetMsgFromDbInRange(ctx context.Context) getMsgFromDbInRangeFn {
	...
	return func(ctx context.Context, args GetMsgFromDbInRangeArgs, reply *GetMsgFromDbInRangeReply) error {
		...
		rows, err := rethinkdb.Table(config.RethinkMsgDb).
			...
			Run(rdb)
		if err != nil {
			r.GetLogger().Error("GetMsgFromDbInRange rethinkdb.GetMsgFromDbInRange Err:", fmt.Sprintf("args: %+v, err: %v", args, err))
			return err
		}
		defer rows.Close()
		if err := rows.All(&msgs); err != nil {
			r.GetLogger().Error("GetMsgFromDbInRange rethinkdb.Bind To Struct Err:", fmt.Sprintf("args: %+v, err: %v", args, err))
			return err
		}
...
func (r *rpcxServer) GetLast30MsgFromDb(ctx context.Context) getLast30MsgFromDbFn {
	...
	return func(ctx context.Context, args GetLast30MsgFromDbArgs, reply *GetLast30MsgFromDbReply) error {
		...
		rows, err := rethinkdb.Table(config.RethinkMsgDb).
			...
			Run(rdb)
		if err != nil {
			r.GetLogger().Error("GetLast30MsgFromDb rethinkdb.GetLast30MsgFromDb Err:", fmt.Sprintf("args: %+v, err: %v", args, err))
			return err
		}
		defer rows.Close()
		if err := rows.All(&msgs); err != nil {
			r.GetLogger().Error("GetLast30MsgFromDb rethinkdb.Bind To Struct Err:", fmt.Sprintf("args: %+v, err: %v", args, err))
			return err
		}
		if args.Sort == contant.Asc {
			sort.Slice(msgs, func(i, j int) bool {
...
func (r *rpcxServer) GetThe30MsgBeforeTheId(ctx context.Context) getThe30MsgBeforeTheIdFn {
	...
	return func(ctx context.Context, args GetThe30MsgBeforeTheIdArgs, reply *GetThe30MsgBeforeTheIdReply) error {
		...
		rows, err := rethinkdb.Table(config.RethinkMsgDb).
			...
			Run(rdb)
		if err != nil {
			r.GetLogger().Error("GetThe30MsgBeforeTheId rethinkdb.GetThe30MsgBeforeTheId Err:", fmt.Sprintf("args: %+v, err: %v", args, err))
			return err
		}
		defer rows.Close()
		if err := rows.All(&msgs); err != nil {
			r.GetLogger().Error("GetThe30MsgBeforeTheId rethinkdb.Bind To Struct Err:", fmt.Sprintf("args: %+v, err: %v", args, err))
			return err
		}
		if args.Sort == contant.Asc {
			sort.Slice(msgs, func(i, j int) bool {
...
func (r *rpcxServer) GetThe30MsgAfterTheId(ctx context.Context) getThe30MsgAfterTheIdFn {
	...
	return func(ctx context.Context, args GetThe30MsgAfterTheIdArgs, reply *GetThe30MsgAfterTheIdReply) error {
		...
		rows, err := rethinkdb.Table(config.RethinkMsgDb).
			...
			Run(rdb)
		if err != nil {
			r.GetLogger().Error("GetThe30MsgAfterTheId rethinkdb.GetThe30MsgBeforeTheId Err:", fmt.Sprintf("args: %+v, err: %v", args, err))
			return err
		}
		defer rows.Close()
		if err := rows.All(&msgs); err != nil {
			r.GetLogger().Error("GetThe30MsgAfterTheId rethinkdb.Bind To Struct Err:", fmt.Sprintf("args: %+v, err: %v", args, err))
			return err
		}
		if args.Sort == contant.Asc {
			sort.Slice(msgs, func(i, j int) bool {
